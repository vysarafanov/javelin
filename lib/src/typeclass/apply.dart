...
extension ApplyExt<F> on Apply<F> {
  Kind<F, Tuple2<A, B>> product<A, B>(Kind<F, A> fa, Kind<F, B> fb) => ap(
      fb, map<A, Tuple2<A, B> Function(B)>(fa, (A a) => (B b) => Tuple2(a, b)));

  Kind<F, Tuple3<A, B, C>> product2<A, B, C>(
          Kind<F, Tuple2<A, B>> fab, Kind<F, C> fc) =>
      ap(
          fc,
          map<Tuple2<A, B>, Tuple3<A, B, C> Function(C)>(
              fab, (Tuple2<A, B> ab) => (C c) => Tuple3(ab.a, ab.b, c)));
